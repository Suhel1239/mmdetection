...
from mmengine.runner import load_checkpoint  # We'll avoid using this for loading weights here

from mmengine.utils import get_classes
from mmengine.hooks import init_default_scope

# For patching safe globals during torch.load
...
def init_detector(
    ...
    # Patch safe globals before loading checkpoint weights
    add_safe_globals([HistoryBuffer, _reconstruct, dtype])
    ...
    if checkpoint is None:
        ...
        # Manually load checkpoint with patched safe globals
        checkpoint_data = torch.load(checkpoint, map_location='cpu')

        # Extract state_dict or fallback to entire checkpoint_data
